...
/** [label, response] for a branching flow option (messaging). */
export type FlowOpt = [string, string]

export interface PushOverride {
  flow: { intro: string; opts: string[][] }
}
...
export interface Pack {
  ...
  flow: { intro: string; opts: FlowOpt[] }
  push?: PushOverride
  email?: EmailOverride
}
...
export const cap = (x: string): string => (x ? x.charAt(0).toUpperCase() + x.slice(1) : x)
...
export const PACKS: Record<string, Pack> = {
  ...
  d2c: { brand: 'GlowLab', emoji: '✨', offer: 'Buy 1 Get 1 on bestsellers', url: 'glowlab.co', hero: 'beauty', orderNoun: 'order', orderId: '4471', code: 'GLOW20', otpUse: 'login', reminder: 'time to restock your Vitamin C serum', feedbackQ: "How's your skin feeling?", carousel: [['Vit-C Serum', '$119'], ['Night Cream', '$149'], ['SPF 50', '$89']],
    flow: { intro: '✨ Running low, Alex? Your *Vitamin C Serum* usually lasts 30 days — time to restock.', opts: [['Reorder now', '🛒 Added to cart with 15% off (GLOW20). Checkout: glowlab.co/cart'], ['Try something new', 'Great! Here are 3 bestsellers loved by your skin type.'], ['Not yet', "No problem — we'll check in next week. 💛"]] },
  ...
  ott: { brand: 'Streamly', emoji: '🎬', offer: '50% off your next month', url: 'streamly.tv', hero: 'ott', orderNoun: 'subscription', orderId: 'SUB-771', code: 'COMEBACK50', otpUse: 'login', reminder: '3 new seasons landed in your watchlist', feedbackQ: 'Enjoying the new releases?', carousel: [['Originals', 'New'], ['Blockbusters', '4K'], ['Live comedy', 'Tonight']],
    flow: { intro: 'We miss you, Alex 🍿 *3 new seasons* just landed in your watchlist. Come back for *50% off*?', opts: [['Claim 50% off', '🎉 Done! Your next month is half price. Jump back in: streamly.tv/home'], ["What's new?", 'This week: 3 Originals, 2 blockbusters & a live comedy special 🎤'], ['Cancel updates', 'Got it — no more promos. Manage anytime in Settings.']] },
  ...
  delivery: { brand: 'QuickBite', emoji: '🍔', offer: 'Free delivery all weekend', url: 'quickbite.app', hero: 'food', orderNoun: 'order', orderId: '5521', code: 'FREEDEL', otpUse: 'login', reminder: 'hungry? your favourites are one tap away', feedbackQ: 'How was your meal?', carousel: [['Burger Yard', '20% off'], ['Sushi Bar', 'Free miso'], ['Pizza Co', 'Buy 1 Get 1']],
    flow: { intro: 'Your order *#5521* from Burger Yard is being prepared 👨‍🍳 Need anything?', opts: [['Track rider', '🛵 Mike is 8 min away. Track live: quickbite.app/track/5521'], ['Add cutlery', 'Added! Your rider will bring cutlery. 🍴'], ['Report an issue', "Sorry about that! Reply with the item and we'll refund it."]] },
    push: { flow: { intro: 'Your order #5521 from Burger Yard is on the way 🛵 Mike is 8 min away.', opts: [['Track rider'], ['Call rider'], ['Dismiss']] } },
...
/** Merge a pack's base fields with its `email` override into a flat EmailPack (Gmail). */
export function emailPackFor(ctxId: string): EmailPack {
  const p = PACKS[ctxId] || PACKS.fashion
  const e = p.email || ({ from: 'hello@' + p.url, accent: '#111111', products: p.carousel, total: '—' } as EmailOverride)
  return {
    brand: p.brand,
    from: e.from,
    accent: e.accent,
    offer: e.offer || p.offer,
    code: e.code || p.code || 'HELLO10',
    url: e.url || p.url,
    orderId: p.orderId,
    otpUse: e.otpUse || p.otpUse,
    reminder: p.reminder,
    products: e.products,
    total: e.total,
  }
...
/** The push flow (notify) for a ctx — its own one-liner override, or derived from the messaging flow. */
export function pushFlowFor(ctxId: string): { intro: string; opts: string[] } {
  const p = PACKS[ctxId] || PACKS.fashion
  if (p.push) return { intro: p.push.flow.intro, opts: p.push.flow.opts.map((o) => o[0]) }
  return { intro: p.flow.intro.replace(/\n+/g, ' '), opts: p.flow.opts.map((o) => o[0]) }
}
...
/** Map a free-text industry label onto a known vertical. Returns {industryId, subId} or null. */
export function resolveIndustry(label: string): { industryId: string; subId: string | null } | null {
  ...
  if (!key) {
    for (const ind of INDUSTRIES) {
      if (cands.includes(ind.id)) { key = ind.id; break }
      for (const s of ind.subs) if (cands.includes(s[0])) { key = s[0]; break }
      if (key) break
    }
  }
  if (!key) return null
  for (const ind of INDUSTRIES) {
    if (ind.id === key) return { industryId: ind.id, subId: ind.subs.length ? ind.subs[0][0] : null }
